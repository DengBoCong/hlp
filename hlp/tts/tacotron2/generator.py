...
# train数据生成器
def train_generator(wav_name_list, batch_size, csv_dir, tokenizer, wave_train_path, config):
    ...
    while True:
        ...
        for idx in range(batchs):
            ...
            wav_tensor, input_length = dataset_mel(
                wave_train_path, config.max_input_length, wav_name_list2, config
            )
...
# 测试数据生成器
def test_generator(wav_name_list, batch_size, csv_dir, tokenizer, wave_test_path, config):
    # generator只能进行一次生成，故需要while True来进行多个epoch的数据生成
    while True:
        batchs = len(wav_name_list) // batch_size
        for idx in range(batchs):
            ...
            # 取音频数据
            wav_tensor, input_length = dataset_mel(
                wave_test_path, config.max_input_length, wav_name_list2, config
            )
