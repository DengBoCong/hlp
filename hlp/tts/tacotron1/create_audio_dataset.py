...
def precess_audio(meta_file, wave_file_dir, output_dir='./data/'):
    ...
    for filename in tqdm(dot_wav_filenames):
        ...
        decod_inp = decod_inp[:, -N_MEL:]  # 取后80
        print('decod_inp: ', decod_inp.shape)

        dim0_mel_spectro = mel_spectro.shape[0]
        dim1_mel_spectro = mel_spectro.shape[1]
        padded_mel_spectro = np.zeros((MAX_MEL_TIME_LENGTH, dim1_mel_spectro))  # 时间长度一致
        ...
        padded_decod_input = np.zeros((MAX_MEL_TIME_LENGTH, dim1_decod_inp))  # 时间长度一致
        ...
        padded_spectro = np.zeros((MAX_MAG_TIME_LENGTH, dim1_spectro))  # 时间长度一致
