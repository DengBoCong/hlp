    ...
    *单轮对话数据集处理模块*
    ...
    for file_fn in standby_data:
        ...
        with open(file_fn, 'r', encoding='utf-8') as tokenized_file, open(combine_data, 'a',
            ...
            lines = tokenized_file.read().strip().split("\n")
            for line in lines:
                line.strip('\n').replace('/', '')
...
def dispatch_tokenized_func_dict_single(operator: str, raw_data: str, tokenized_data: str, if_remove: bool = True):
    """
    *单轮对话数据集处理模块*
...
def raw_to_tokenized_and_combine_single(standby_data: dict, combine_data: str, if_save_tokenized: bool = False):
    ...
    *单轮对话数据集处理模块*
    ...
    if if_save_tokenized:
        combine_tokenized_data_single(standby_data=standby_data.values(), combine_data=combine_data)
    else:
