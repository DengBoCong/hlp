...
class TaskChatter(Chatter):
    ...
    def train(self, checkpoint, dict_fn, data_fn, start_sign, end_sign, max_train_data_size):
        input_tensor, target_tensor, lang_tokenizer = \
            _data.load_dataset(dict_fn=dict_fn, data_fn=data_fn, start_sign=start_sign,
        ...
        data_load = _data.load_data(_config.dialogues_train, _config.max_length, _config.database, _config.ontology,
                                    lang_tokenizer.word_index, _config.kb_indicator_len)
        for epoch in range(_config.epochs):
            ...
            while (True):
                ...
                if data_load.cur == 0:
                    break
                state_tracker_hidden = tf.zeros((usr_utts.shape[0], 150))
                model = task.task(_config.units, data_load.onto,
                                  _config.vocab_size, _config.embedding_dim, _config.max_length, state_tracker_hidden)
                ...
                with tf.GradientTape() as tape:
                    ...
                    for key in state_preds:
                        temp = tf.argmax(state_preds[key], axis=-1).numpy()
                        pred.append(tf.cast(temp, dtype=tf.int64))
                        real.append(state_gt[key])
...
def main():
    ...
    if options.type == 'train':
        chatter.train(checkpoint=_config.task_train_data,
                      dict_fn=_config.dict_fn,
