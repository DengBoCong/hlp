...
def inform_slot_tracker(n_choices, name="inform_slot_tracker"):
    """
    ...
    """
    inputs = tf.keras.Input(shape=(None,), name="inform_slot_tracker_inputs")
    outputs = tf.keras.layers.Dense(units=n_choices)(inputs)
    return tf.keras.Model(inputs=inputs, outputs=outputs, name=name)


def request_slot_tracker(name="request_slot_tracker"):
    """
    ...
    """
    inputs = tf.keras.Input(shape=(None,), name="request_slot_tracker_inputs")
    outputs = tf.keras.layers.Dense(units=2)(inputs)
    return tf.keras.Model(inputs=inputs, outputs=outputs, name=name)


def state_tracker(units, hidden, name="state_tracker"):
    inputs = tf.keras.Input(shape=(None,), name="state_tracker_inputs")
    output, state = tf.keras.layers.GRU(units, return_sequences=True,
                                        return_state=True,
                                        dropout=0.9,
                                        recurrent_initializer='glorot_uniform')(inputs, initial_state=hidden)
